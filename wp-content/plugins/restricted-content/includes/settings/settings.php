...
<div class="wrap rsc_outside_wrap nosubsub rsc_outside_<?php echo $tab; ?>">
  <h1></h1>
  ...
  <div class="rsc-settings-header">
    <?php if(!rsc_iw_is_wl()){ ?>
      <div class="rcs-restrict-logo">
        <img src="<?php echo $rsc->plugin_url; ?>assets/images/restrict-logo@2x.png" width="70" />
        <div class="rsc-version">v <?php echo $rsc->version; ?></div>
      </div>
    <?php } ?>

    <div class="rsc-header-title">
      <h1 class="<?php echo $rsc_heading_class; ?>"><?php echo isset($section['title']) ? $section['title'] : '';?></h1>
      <span><?php echo isset($section['subtitle']) ? $section['subtitle'] : ''; ?></span>
    </div>
    <?php
    if(!rsc_iw_is_wl()){
      ?>
      <div class="rsc-documentation-link">
        <a href="<?php echo esc_attr(rsc_get_documentation_link($tab));?>" target="_blank" class="rsc-documentation-button"><?php echo sprintf(__('View %s Documentation', 'rsc'), (isset($menus[$tab]) && !empty($menus[$tab]) && $tab !== 'welcome') ? $menus[$tab] : '');?></a>
      </div>
    <?php } ?>
  ...
  if (isset($_POST['submit'])) {
    ?>
    <div id="message" class="updated fade"><p><?php _e('Settings saved successfully.', 'rsc'); ?></p></div>
    <?php
  }
  if (version_compare(phpversion(), '5.3', '<')) {
    ?>
    <div id="rsc_php_53_version_error" class="error" style=""><p><?php echo sprintf(__('Your current version of PHP is %s and recommended version is at least 5.3. You should contact your hosting company and %sask for upgrade%s.', 'rsc'), phpversion(), '<a href="https://wordpress.org/about/requirements/">', '</a>') ?></p></div>
      <?php
    }
    ?>
